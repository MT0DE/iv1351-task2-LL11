...
ALTER TABLE available ADD CONSTRAINT FK_available FOREIGN KEY (instructor_id) REFERENCES instructor (person_id);
ALTER TABLE known_instrument ADD CONSTRAINT FK_known_instrument FOREIGN KEY (instructor_id) REFERENCES instructor (person_id);
...
SELECT id, instructor_id, payment_id, locale, booking_date, start_time, end_time, difficulty, instrument
FROM booking 
LEFT JOIN group_lesson ON booking.id = group_lesson.booking_id
LEFT JOIN ensemble ON group_lesson.booking_id = ensemble.group_lesson_id 
ORDER BY id ASC;
